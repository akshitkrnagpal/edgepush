...
`OPERATOR.md` is the production runbook for running a hosted tier.
...
1. **Integration tests** for the send path, dispatch consumer, and
   account-delete cascade. v0.1 ships 70 pure-function unit tests but
   no D1-backed integration tests. See `CHANGELOG.md` "Known
   not-yet-landed" for context. Vitest + `@cloudflare/vitest-pool-workers`
   is the path.
2. **Mobile SDK examples**: React Native, native iOS, native Android.
   ...
   section in `apps/web/src/app/docs/page.tsx`.
...
5. **Webhook delivery retry queue.** Today webhook failures are logged
   to `worker_errors` but not retried. A dedicated webhook queue
   (separate from `edgepush-dispatch`) with its own consumer and DLQ
   is the right shape. Don't just add inline retries to the dispatch
   consumer.
...
- `apps/api` (server) and `apps/web` (dashboard) are
