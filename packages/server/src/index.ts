...
// RateLimiter is NOT exported from the main entry because it imports
// from cloudflare:workers (DurableObject base class) which breaks
// Next.js builds. Import it from @edgepush/server/do instead.
// export { RateLimiter } from "./rate-limiter";

export { handleScheduled } from "./cron";
export { handleQueue, handleDlq } from "./dispatch";
export { handleWebhookQueue, handleWebhookDlq } from "./webhook-consumer";
export type { Env, AppContext, DispatchJob, WebhookJob } from "./types";
...
// CLI token endpoint: returns the session token for CLI OAuth login.
// The CLI uses this token as a Bearer token for admin commands.
app.get("/api/cli-token", async (c) => {
  const auth = createAuth(c.env);
