...
export interface Env {
  ...
  /**
   * Demo mode. Set to "true" on the demo instance (demo.edgepush.dev).
   * Adds extra abuse-prevention limits on top of the normal rate limiter:
   *   - rate limit capped at 50/min regardless of per-app config
   *   - max 2 apps per user (enforced in dashboard create-app route)
   *   - all data wiped daily via external cron
   */
  DEMO_MODE?: string;
