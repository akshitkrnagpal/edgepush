...
// appId is a server-generated UUID (crypto.randomUUID()). Reject anything
// else up front so a tainted identifier from a support ticket can't flow
// into a SQL string or the shell.
...
if (!UUID_RE.test(appId)) {
  console.error("[inspect] appId must be a UUID (8-4-4-4-12 hex).");
  process.exit(1);
}
...
// execFileSync passes args directly to the wrangler binary without going
// through /bin/sh, so nothing in `sql` is shell-interpreted.
function run<T = unknown>(sql: string): T[] {
  const raw = execFileSync(
    "wrangler",
    ["d1", "execute", binding, scope, "--json", "--command", sql],
