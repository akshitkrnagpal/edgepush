...
// Both IDs are server-generated UUIDs (crypto.randomUUID()); reject anything
// else at entry so tainted input from a support ticket can't flow into SQL
// or the shell.
...
if (!UUID_RE.test(appId) || !UUID_RE.test(messageId)) {
  console.error(
    "[replay-dlq] appId and messageId must be UUIDs (8-4-4-4-12 hex).",
  );
  process.exit(1);
}
...
// execFileSync avoids /bin/sh and passes args directly to the process;
// nothing in `args` is shell-interpreted.
function wrangler(subArgs: string[]): string {
  return execFileSync("wrangler", subArgs, {
