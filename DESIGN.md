...
- **Pulse:** The hero status chip pulses in accent orange (`box-shadow` pulse, 1.8s ease-out infinite). Nothing else pulses.
...
- **CSS:** Both `apps/web/src/app/globals.css` (marketing) and `apps/app/src/app/globals.css` (dashboard) define the design tokens.
...
- **Both apps** (`apps/web` for marketing, `apps/app` for dashboard) follow this design system.
